...
func TestForkId13DencunConfig(t *testing.T) {
	...
	if config.DevnetBlock != 1 {
		t.Errorf("ForkId13DencunConfig.DevnetBlock = %d, want 1", config.DevnetBlock)
	}
}
...
func TestForkConfigsRegistry(t *testing.T) {
	...
	if config.DevnetBlock != 1 {
		t.Errorf("forkConfigs[ForkId13Dencun].DevnetBlock = %d, want 1", config.DevnetBlock)
	}
}
...
func TestGetForkBlock(t *testing.T) {
	...
	tests := []struct {
		...
		{"Local ForkId13Dencun", LocalNetwork, ForkId13Dencun, 1},
