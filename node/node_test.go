...
func TestNodeCloseClosesDB(t *testing.T) {
	...
	db, err := OpenDatabase(context.Background(), stack.Config(), kv.SentryDB, "", false, false, logger)
	if err != nil {
...
func TestNodeOpenDatabaseFromLifecycleStop(t *testing.T) {
	...
	stack.RegisterLifecycle(&InstrumentedService{
		stopHook: func() {
			db, err := OpenDatabase(context.Background(), stack.Config(), kv.ChainDB, "", false, false, logger)
			if err != nil {
				t.Fatal("can't open DB:", err)
			}
			db.Close()
