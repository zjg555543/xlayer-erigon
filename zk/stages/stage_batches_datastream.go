...
// StartReadOptimized uses the new CmdStartBookmarkBatch command to reduce TCP calls from 4 to 1
func (r *DatastreamClientRunner) StartReadOptimized(errorChan chan struct{}) error {
	r.dsClient.RenewEntryChannel()
	if r.isReading.Load() {
		return fmt.Errorf("tried starting datastream client runner thread while another is running")
	}
	...
	go func() {
		...
		log.Info(fmt.Sprintf("[%s] Started downloading L2Blocks routine (optimized) ID: %d", r.logPrefix, routineId))
		defer log.Info(fmt.Sprintf("[%s] Ended downloading L2Blocks routine (optimized) ID: %d", r.logPrefix, routineId))

		r.isReading.Store(true)
		defer r.isReading.Store(false)

		if err := r.dsClient.ReadAllEntriesToChannelOptimized(); err != nil {
			time.Sleep(1 * time.Second)
			errorChan <- struct{}{}
			log.Warn(fmt.Sprintf("[%s] Error downloading blocks from datastream (optimized)", r.logPrefix), "error", err)
		}
	}()
